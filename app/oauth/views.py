...
def discord():
    scope = request.args.get("scope", "identify guilds.join")
...
def callback():
    ...
    payload = {
        ...
        "roles": [app.config["FREESIDE_ROLE"]],
    }
    headers = {
        "Content-Type": "application/json",
        "Authorization": "Bot " + app.config["BOT_TOKEN"],
    }

    invite = requests.put(
        app.config["API_BASE_URL"]
        + "/guilds/{}/members/{}".format(app.config["GUILD_ID"], duser["id"]),
        json=payload,
        headers=headers,
    )
    ...
    if invite.status_code == 201:
        ...
        role = requests.put(
            ...
            + "/guilds/{}/members/{}/roles/{}".format(
                app.config["GUILD_ID"], duser["id"], app.config["FREESIDE_ROLE"]
            ),
            headers=headers,
        )
